...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate database webhook sync processing
    setTimeout(() => {
      if (email === "error@devops.com") {
        setGlobalError("Clerk Sync Webhook failed! Local profile could not be created in PostgreSQL.");
        setIsLoading(false);
      } else {
        setLoadingText("Synchronizing profile...");
        setTimeout(() => {
          setIsSuccess(true);
          setIsLoading(false);
          // Simulating redirect
      ...
      <div className="w-full max-w-[440px] bg-white border-2 border-black rounded-sketchy shadow-flat-offset p-8 flex flex-col relative z-10">
        ...
        {isSuccess && (
          <div className="absolute inset-0 bg-white/95 rounded-sketchy z-40 flex flex-col items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="font-cursive text-3xl font-bold text-center mb-2">Account Created!</h2>
            <p className="font-sans text-sm text-secondary text-center">Setting up your whiteboard dashboard...</p>
          </div>
        )}
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={isLoading}
                className={`w-full font-sans text-sm pl-10 pr-4 py-2.5 bg-neutral-bg border-2 border-black rounded-sketchy-sm focus:outline-none focus:ring-2 focus:ring-tertiary focus:bg-white transition-all disabled:opacity-50 ${
        ...
        <p className="font-sans text-xs text-center text-secondary mt-8 select-none">
          ...
          <Link href="/login" className="text-tertiary font-bold hover:underline hover:text-tertiary-hover">
            Sign In
          </Link>
        </p>
