...
## Current Status
- **Status:** **Feature 2.6 (Search & Team Directory) Implemented**. Applied `tsvector` GIN full-text search index on `tasks`. Created `actions/search.ts` (parallel project/task/member search) and `actions/team.ts` (workload-aware team directory). Built `GlobalSearchModal` with debounced input, keyboard navigation (↑↓/Enter/Esc), and Cmd+K shortcut. Built `SearchTrigger` sidebar button and `TeamDirectory` member grid. Created `/team` page. Updated `Sidebar` with search trigger and Team Directory nav link.
