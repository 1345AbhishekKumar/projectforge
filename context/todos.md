...
- [x] **SSO Auth Handshake:** Perform Okta / Entra ID logins. Verify JWT verification succeeds and redirects user into active workspace.
- [x] **JIT Provisioning:** Verify a new employee logging in via SSO has their profile, organization membership, and default role auto-created.

### Feature 4.16: Distributed Architecture & Infrastructure

- [x] **API Gateway Routing:** Verify that request routes are properly split and dispatched to independent services (Auth, Org, Project, etc.).
- [x] **Event Bus Delivery:** Verify that event dispatching works asynchronously and failures are handled with retries and dead-letter queues.
- [x] **Telemetry & Tracing:** Run load tests. Verify OpenTelemetry captures distributed traces across all service boundaries and lists them in Grafana.
